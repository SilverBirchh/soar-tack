...
export default Ember.Component.extend({
  ...
  checkWinner() {
    ...
    ['top', 'middle', 'bottom'].forEach((row) => {
      ...
      ['right', 'left', 'middle'].forEach((cell) => {
        rowCount += (this.get(`map.${row}.${cell}.shape`) === this.get('playerOne.shape')) ? 1 : 0;
        rowCount += (this.get(`map.${row}.${cell}.shape`) === this.get('playerTwo.shape')) ? -1 : 0;
    ...
    ['right', 'left', 'middle'].forEach((col) => {
      ...
      ['top', 'middle', 'bottom'].forEach((cell) => {
        colCount += (this.get(`map.${cell}.${col}.shape`) === this.get('playerOne.shape')) ? 1 : 0;
        colCount += (this.get(`map.${cell}.${col}.shape`) === this.get('playerTwo.shape')) ? -1 : 0;
    ...
    ['top', 'middle', 'bottom'].forEach((row) => {
      ['right', 'left', 'middle'].forEach((cell) => {
        if (`${row}.${cell}` === 'top.left' || `${row}.${cell}` === 'middle.middle' || `${row}.${cell}` === 'bottom.right') {
          diagCount += (this.get(`map.${row}.${cell}.shape`) === this.get('playerOne.shape')) ? 1 : 0;
          diagCount += (this.get(`map.${row}.${cell}.shape`) === this.get('playerTwo.shape')) ? -1 : 0;
    ...
    ['top', 'middle', 'bottom'].forEach((row) => {
      ['right', 'left', 'middle'].forEach((cell) => {
        if (`${row}.${cell}` === 'top.right' || `${row}.${cell}` === 'middle.middle' || `${row}.${cell}` === 'bottom.left') {
          antiDiagCount += (this.get(`map.${row}.${cell}.shape`) === this.get('playerOne.shape')) ? 1 : 0;
          antiDiagCount += (this.get(`map.${row}.${cell}.shape`) === this.get('playerTwo.shape')) ? -1 : 0;
  ...
  reset() {
    $('.row-container div').each((cell) => {
      $('.row-container div').eq(cell).removeClass(`${this.get('playerOne.shape')} ${this.get('playerTwo.shape')}`);
    });
    ['top', 'middle', 'bottom'].forEach((row) => {
      ['right', 'left', 'middle'].forEach((cell) => {
